"""
Entry point; it sets up parameters based on the -dev flag, initializes the model, and runs the main capture and detection loop.
When an object is confirmed, it calls the tray rotation function from tray.py, then recalibrates the camera for post-rotation.
"""
...
while True:
        ...
        if mean_diff > diff_threshold:
            ...
            if not confirmed_object:
                ...
                if elapsed >= WAIT_TIME and union_box:
                    ...
                    cv2.putText(frame, f"Accumulating {len(roi_accumulation)}/{ACCUM_COUNT}", (roi_left + x, roi_top + y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    ...
    cv2.putText(frame, f"Diff: {mean_diff:.1f}" if detection_enabled else "", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
