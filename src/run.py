"""
Entry point; it sets up parameters based on the -dev flag, initializes the model,
and runs the main capture and detection loop.
When an object is confirmed, it calls the tray rotation function from tray.py,
then recalibrates the camera for post-rotation (in production only).
"""
...
# Initialize the model (default is quantized for improved performance)
...
# In production mode, accumulate multiple ROI frames for robust prediction.
roi_accumulation = []
ACCUM_COUNT = 5
target_shape = None
...
while True:
    ...
    if detection_enabled:
        ...
        if mean_diff > diff_threshold:
            ...
            if points:
                ...
                # --- Expanded bounding box by 25% for better coverage ---
                pad_frac = 0.25
                ...
                cv2.rectangle(frame, (roi_left + x, roi_top + y),
                              (roi_left + x + w, roi_top + y + h), (0, 255, 0), 2)
            ...
            if not confirmed_object:
                ...
                if elapsed >= WAIT_TIME and union_box:
                    ...
                    if '-dev' in sys.argv:
                        # In dev mode, use a single frame for prediction.
                        ...
                        # In production mode, accumulate multiple frames and then use median.
    ...
    # Display status on frame (top-left)
    cv2.putText(frame, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, status_color, 2)
    cv2.putText(frame, f"Diff: {mean_diff:.1f}" if detection_enabled else "",
                (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)

    # Compute and display frame rate in top-right corner.
    ...
    if confirmed_object and detection_enabled:
        ...
        if last_category is not None and last_category in ["plastic", "paper", "food waste", "metal"]:
            simulate_tray_rotation(last_category, frame)
        else:
            print("Predicted category is invalid; skipping tray rotation.")
        if '-dev' not in sys.argv:
            baseline_roi = calibrate_background(cap, roi_top, roi_left, roi_size, POST_ROTATION_CALIBRATION)
        confirmed_object = False
        detection_enabled = True
